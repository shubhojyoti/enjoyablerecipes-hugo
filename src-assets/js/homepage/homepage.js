...
(function() {
    ...
    var slideTimer = 5000;
    var intervalId;
    var makeRadioChecked = function makeRadioChecked(idx) {
        var inputs = document.querySelectorAll(".slider_labels input[type='radio']");
    ...
    var selectSlide = function selectSlide(event) {
        var slides = document.querySelector(".slider_images");
        if (slides !== undefined && slides !== null) {
            ...
            if (null === tar || undefined === tar) {
                var nextslide = -1;
                var currentslide = -1;
                var tot = slides.children.length;
                for (var i=1; i<=tot; i++) {
                ...
                var idx = tar.id.slice(-1);
                for (var j=1; j<=slides.children.length; j++) {
    ...
    var genRandom = function genRandom(max) {
        return Math.floor(Math.random() * max)
    };

    var createSliderHtml = function createSliderHtml(data, numbers, max) {
        var html = "";
        for (var i=0; i<numbers.length; i++) {
            var caption = data[numbers[i]].recipeShortDesc;
            var title = data[numbers[i]].recipeTitle;
            var url = data[numbers[i]].recipeUrl;
            var curr = data[numbers[i]].recipeMainImage;
            while (caption === "" || caption === null) {
                var num = genRandom(max);
    ...
    var getRandomPosts = function getRandomPosts() {
        ...
        request.onload = function() {
          if (request.status >= 200 && request.status < 400){
            ...
            total = data.length;
            var numbers = [genRandom(total), genRandom(total), genRandom(total), genRandom(total), genRandom(total)];
    ...
    var slideChanger = function slideChanger() {
        var obj = document.querySelectorAll(".slider_labels input[type='radio']");
